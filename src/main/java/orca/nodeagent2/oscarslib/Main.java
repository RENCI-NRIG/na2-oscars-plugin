...
public class Main implements Plugin {
	...
	public void join2() {
		...
		Driver d = new Driver(all.get(IDC_PROP), 
				all.get(TRUSTSTORE_PROP),
				all.get(KEYSTORE_PROP),
				all.get(ALIAS_PROP),
				all.get(PASSWORD_PROP));

		System.out.println("THERE");

		try {
			// convert bandwidth and vlan tags to ints
			int bw = Integer.parseInt(all.get(BW_PROP));
			int tagA = Integer.parseInt(all.get(TAGA_PROP));
			int tagZ = Integer.parseInt(all.get(TAGZ_PROP));

			System.out.println("Creating OSCARS reservation from " + all.get(EPA_PROP) + "/" + tagA + " to " + all.get(EPZ_PROP) + "/" + tagZ + " with bandwidth " + bw);
			...
			String gri = "fake";
			if (false) {
				gri = d.createReservationPoll(configProperties.get(DESCRIPTION_PROP), 
						new Date(), cal.getTime(), bw, 
						all.get(EPA_PROP), tagA, 
						all.get(EPZ_PROP), tagZ, 
						POLL_INTERVAL);
			}

			// save properties
			Properties joinProps = new Properties();
			joinProps.putAll(all);
			joinProps.put(GRI_PROP, gri);

			System.out.println("Created OSCARS reservation " + gri);

			String resId = gri + "---" + all.get(EPA_PROP) + "---" + tagA + "---" + all.get(EPZ_PROP) + "---" + tagZ;
